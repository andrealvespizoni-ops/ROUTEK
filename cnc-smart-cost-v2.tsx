...
const STORAGE_KEY = "cnc_config_v6";
...
const GEOMETRIAS = [
  ...
  { id:"especiais",    label:"Especiais",        icon:"✦", campos:["largura","altura"], extra:"+25% projeto",
    refL:500, refA:500, progS:300, maqS:120 },
...
function calcTempos(item) {
  ...
  const progS = geo.progS * fator * qtd;
  const maqS  = geo.maqS  * fator * qtd;
...
function calcOrcamento(form, config) {
  ...
  const custoMO         = (tempoProjetoH + tempoSetupH + tempoUsinagemH) * num(config.valorHoraOperador);
  ...
  const areaPerda       = chapasNec * CHAPA_AREA - areaTotal;

  return { tempoProjetoH, tempoSetupH, tempoUsinagemH, consumoKWh, custoMO, custoEnergia, custoManutencao, custoMat, custoBase, valorFinal, lucro, areaTotal, chapasNec, areaPerda };
}
...
function ConfigScreen({ config, setConfig, onSave }) {
  ...
  const ok = ["valorHoraOperador","potenciaSpindle","potenciaExtras","tarifaEnergia","custoManutencaoHora","margem"].every(k => config[k] !== "");
  ...
  return (
    <div>
      ...
      <Card title="Mão de Obra" icon="👷" accent="#7c3aed">
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:10 }}>
          {GEOMETRIAS.map(g => (
            <Chip key={g.id} active={item.geometria===g.id} onClick={() => set("geometria")(g.id)} color={g.id==="especiais"?"#f59e0b":g.muxarabi?"#10b981":"#7c3aed"} small>
              ...
              {g.muxarabi && <div style={{ fontSize:8,color:"#10b981" }}>{g.progMin+g.maqMin}min/m²</div>}
            </Chip>
          ))}
        </div>
      </Field>
          ...
          {!geo?.muxarabi && (
            <Field label="Tipo de corte" small>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                ...
                <Chip active={item.ferramenta==="premium"} onClick={() => set("ferramenta")("premium")} color="#f59e0b" small>⭐ Premium · +50% proj</Chip>
              </div>
            </Field>
          )}
  ...
  return (
    <div>
      ...
      <Card title="Material" icon="📦" accent="#f59e0b">
        ...
        {form.donoDaMaterial === "cliente" && (
          <div style={{ background:"#0f0f0f",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#bbbbbb" }}>
            Material do cliente — apenas o serviço de corte será cobrado.
          </div>
        )}
      </Card>
      ...
      {result && (
        <>
          <div style={{ background:"linear-gradient(140deg,#111111,#181818)",border:"1px solid #1e1e1e",borderRadius:14,padding:20,marginBottom:16 }}>
            ...
            <Row label="Tempo de projeto"  value={formatSec(result.tempoProjetoH*3600)} />
            <Row label="Tempo de setup"    value={formatSec(result.tempoSetupH*3600)} />
            <Row label="Tempo de usinagem" value={formatSec(result.tempoUsinagemH*3600)} />
            <Row label="Consumo de energia" value={`${result.consumoKWh.toFixed(3)} kWh`} />
            <Row label="Mão de obra" value={BRL(result.custoMO)} />
            ...
            <Row label="Custo base" value={BRL(result.custoBase)} bold />
          </div>
    ...
    return (
      <div>
          ...
          {q.form?.itens?.map((it,i) => {
            ...
            return (
              <div key={i} style={{ padding:"8px 0",borderBottom:"1px solid #222222" }}>
                ...
                <span style={{ fontSize:11,color:"#bbbbbb",marginLeft:8 }}>{largura}×{altura}mm · {it.quantidade} pç · {it.ferramenta==="premium"?"⭐ Premium":"⚙️ Standard"}</span>
              </div>
            );
          })}
        </Card>
        ...
        <Card title="Custos" icon="📋" accent="#e8e8e8">
          <Row label="Mão de obra" value={BRL(q.result?.custoMO)} />
...
export default function App() {
  ...
  const isConfigured = () => ["valorHoraOperador","potenciaSpindle","potenciaExtras","tarifaEnergia","custoManutencaoHora","margem"].every(k => config[k] !== "");
